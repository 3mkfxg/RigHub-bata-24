...
// --- Category Item Counter with Precomputed Single Pass ---
let _categoryCountMap = null;

function buildCategoryCountMap() {
  const map = new Map();
  if (!Array.isArray(window.products)) return map;

  const schemaGroups = window.SCHEMA_GROUPS || SCHEMA_GROUPS || {};
  ...
  for (let i = 0; i < window.products.length; i++) {
    ...
    if (p.subcategory) seenCats.add(p.subcategory);
    if (p.parent_category) seenCats.add(p.parent_category);

    if (Array.isArray(p.categories)) {
      for (let j = 0; j < p.categories.length; j++) {
        if (p.categories[j]) seenCats.add(p.categories[j]);
      }
    }
    if (Array.isArray(p.parent_categories)) {
      for (let j = 0; j < p.parent_categories.length; j++) {
        if (p.parent_categories[j]) seenCats.add(p.parent_categories[j]);
      }
    }

    for (const [parent, subcats] of Object.entries(schemaGroups)) {
      if (Array.isArray(subcats)) {
        for (let k = 0; k < subcats.length; k++) {
          if (seenCats.has(subcats[k])) {
            seenCats.add(parent);
            break;
          }
        }
  ...
  _categoryCountMap = map;
  return map;
}

function countProductsForCategory(catName) {
  if (!catName) return 0;
  if (!_categoryCountMap) {
    buildCategoryCountMap();
  }
  return _categoryCountMap.get(catName) || 0;
}

function clearCategoryCountCache() {
  _categoryCountMap = null;
}
